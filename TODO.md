...
- The `gi` sample does not run on WebGPU: its scene upload trips `BufferUsage::(MapWrite|CopySrc)` missing `CopyDst` (1804 errors/run), which invalidates the cull pipeline and every command buffer, so the frame is blank. Pre-existing and unrelated to material textures -- an A/B with the set-3 path removed produces the identical error signature.
- Three more WebGPU validation errors at gi startup, all in the set-0/scene path: six 'Entry point's stage (Fragment) is not in the binding visibility in the layout (Compute)', one ReadOnlyStorage-vs-Storage buffer type mismatch, and one 'Binding doesn't exist in VkmBindlessBindGroupLayout'.
