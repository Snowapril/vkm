...
- `TestAccelerationStructureMetal.mm` is written and passes standalone (21 assertions: BLAS build, TLAS instance, transform update, recorded rebuild, and both refusal guards) but is **not in the build**: with it registered the whole Metal suite dies in mimalloc during teardown, and the cause is unidentified.
