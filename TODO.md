...
- The Phase 7 convergence gate (`runIndirectConvergenceTest`) is registered on Vulkan only. On Metal it passes alone and passes with validation off, but returns **zero ray hits** under `MTL_DEBUG_LAYER=1` once any earlier test case has run in the same process. Ruled out: the driver (each test case builds its own), descriptor set 1 (a shader that reports `cameraPositionWorld` reads it correctly in both cases), the bottom-level structures (identical sizes, offsets, stride and triangle counts), the top-level structure's size, the argument-buffer entry (written once, with the live structure's `gpuResourceID`), residency registration of the structures and of their scratch and instance buffers, `requestResidency`, barriers around the synchronous build, and the G-buffer and graphics submit (the failure reproduces with neither present). The only measured difference is that `MTLResourceID` values and object addresses shift when another test case has allocated first.
