...
- `VkmScene::buildAccelerationStructures` is unverified past "the build accepted it": zeroing the geometry pool's vertex and index byte offsets leaves the scene test passing, because a wrong-but-in-range address builds over the wrong triangles and nothing traverses the result until Phase 5's ray-query gate exists.
