...
- The raster tier's area lights are diffuse only: LTC's specular needs fitted M^-1 tables this tree does not vendor, and generating them needs an offline optimiser with importance sampling (a hand-written Nelder-Mead probe did not converge -- residual 9.2e2 at roughness 0.25, because a fixed 512-direction quadrature cannot resolve a narrow GGX lobe).
- Karis's representative point was implemented as a stand-in and then removed: it measured 38.7% worst-texel error against exact integration on a roughness-0.4 floor beside a 3x3 panel two units away (against 0.9% on a fully rough one, its intended small-source regime), neither gate discriminated it strongly (deleting it moved the worst texel only 0.9% -> 3.2%), and its branchy closest-point-on-triangle crashed lavapipe's shader compiler in CI. tests/TestAreaLightShared.hpp's numericAreaSpecular was the ground truth for it and went with it; git history has both.
- The deferred pass's fragment shader is close to a size lavapipe's compiler cannot take: adding the area-light specular crashed four Vulkan CI jobs with SIGSEGV in the deferred lighting test, while Metal and MoltenVK ran the same code clean. Anything further added to that shader wants a PSO permutation rather than another branch in the one shader.
