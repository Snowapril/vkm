...
- `VkmResourceUsageBits` (`renderer_common.h`) and `VkmResourceInfo::_usage` are an empty enum and a field nothing reads.
- Metal still opens each render pass with a queue-scoped acquire, so two passes whose declared dependencies do not overlap are ordered against each other anyway; a per-producer `MTLFence` would need the producer's identity carried in `VkmResourceBarrier` and a scope boundary for a subgraph that closes several encoders.
