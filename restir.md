...
- [x] **Direct lighting at the secondary hit** — **NEE landed 2026-08-15**, in exactly the body
      `vkmShadeSecondaryHit()` was reserved for: one deterministic directional sample (the first
      traced-tier consumer of `_lightDirection`) plus one area sample by power CDF from the
      scene-owned light table (`vkm_lights.hlsli` / `light_table.h`), each behind a shadow ray;
      emission collapsed to the path's first vertex so NEE and emission-on-hit never double
      count. Gated analytically: a plane under two unequal emitters matches the Lambert
      edge-sum closed form to 1.78% worst-pixel; the emissive Cornell converges deferred vs
      reference at mean ratio 0.9984. Every pre-existing gate stayed bit-identical — on a
      light-less scene the seam draws zero randoms.
...
- [x] ~~**Test scene.**~~ **Closed 2026-08-15.** Three analytic-or-visual fixtures now exist:
      `gltf_furnace.gltf` (the exact convex-body gate), `gltf_emissive_plane.gltf` (a floor under
      two unequal emitters, whose closed-form Lambert edge-sum is what pins NEE's pdf), and
      `gltf_cornell_emissive.gltf` (the true emissive-light Cornell box the 2026-08-06 note said
      needed NEE to converge and an emissive G-buffer channel to compare against — both now
      exist, and its gate does both).
...
- [x] ~~**Where GI lives.**~~ **Done 2026-08-13: `VkmGiSystem`.** The recommendation, made
      concrete: the engine class owns both techniques and everything they need — probe volume,
      updater, SSGI, probe lighting, the ReSTIR pass with its resize/retirement lifecycle, the
      RT PSO load and the acceleration-structure attempt, the runtime switch — and produces the
      indirect-radiance texture. A caller supplies the shared infrastructure (G-buffer, direct
      target) and calls initialize → prepareScene → resize, then record + advanceFrame per
      frame; the gi sample shrank to exactly the "debug UI and scene setup" this item asked
      for, driving the system through a descriptor and a mutable options struct. Builds on
      WebGPU: the ReSTIR half is runtime-gated, never compiled out.
...
- [x] ~~**Direct lighting strategy.**~~ **Decided 2026-08-04: plain NEE, behind a seam.** Phase 7
      ...
      **Revisited 2026-08-15:** area lights and the reference now exist, so ReSTIR DI is finally
      measurable — it stays future work (Phase 9's plate is full), but the §12 precondition is
      met and the light table's power CDF is exactly the many-light input DI would resample.
...
| 2026-08-15 | — | **Area/emissive lights + NEE, and the emissive G-buffer channel.** The scene now owns a light table — emissive triangles gathered per mesh entry at addModel (object space, before the pool clears its CPU bytes), expanded by final world transforms and CDF-finalized at build, uploaded `[sun-radiance header][N records]` through the material pool's exact bindless pattern, published as two FrameData pad words (ABI-unchanged at 128 B). `KHR_materials_emissive_strength` finally imports, and `vkmShadeSecondaryHit` carries what §12 reserved it for: sun + one power-CDF area sample per vertex, each shadowed, with emission collapsed to the path's first vertex. The area estimator is in the **area measure** (`L·(albedo/π)·cosθ_x·|cosθ_y|/(d²·pdf_A)`) — the analytic plane gate exists because dropping any of that shifts the mean and no convergence test can see it: measured 1.78% worst-pixel against the Lambert edge-sum closed form, sabotage-verified at 1366% (geometry term) and 199% (shadow ray). A fourth G-buffer target carries emission (finally consuming the imported emissive maps), the deferred pass adds it, and the emissive Cornell's camera-blind precondition is asserted through that very channel. Two bugs found on the way, both mine: the 8.3 gate had gone nondeterministic because `unbindPipeline()` is not a barrier under the Metal fence model (explicit `resourceBarrier` per reservoir hand-off restored bit-exact 9.68e-7), and the reservoir buffer was never zeroed, letting the first temporal frame read garbage history. Metal 254/254 throughout; light-less gates bit-identical. |
| 2026-08-13 | — | **GI moved into the engine: `VkmGiSystem`.** Everything technique-shaped left the gi sample — probe volume/updater setup and its ring-budget clamp, SSGI, probe lighting, the ReSTIR pass lifecycle with parity and retirement, the RT PSO load-once and the AS attempt, and the runtime switch — behind initialize → prepareScene → resize and a per-frame record/advanceFrame pair, producing the §5 indirect-radiance texture. The sample now maps gv flags onto a descriptor and an options struct and owns only the shared chain (G-buffer, direct lighting, composite, tone map) plus UI, settling §12's "where GI lives" as recommended. One deliberate ordering change: the probe refresh records after the camera's update rather than before — the two cull views own separate staging and device regions, so nothing orders them. Verified: Metal 246/246 validation-clean, and both techniques' Cornell screenshots through the system match the pre-refactor output. |
...
| 2026-08-16 | — | **Probe placement, and what the Sponza artifact actually was.** Three separate things, reported honestly rather than as one fix. (a) The probe normal bias only ever offset the *cell lookup*: `toProbe` was measured from the raw surface, so the Chebyshev test still compared a surface against the depth its own probe recorded for that same surface, and thin two-sided geometry read as self-occluded at any bias. It now starts at the biased point, and the bias is a fraction of the smallest probe spacing rather than a world constant (0.25 units is a quarter of a room in one scene and nothing in a 3721-unit Sponza). With screenshot runs made deterministic — a stray cursor drag used to move the camera mid-run, which is what made an earlier A/B meaningless — the honest result is that this does **not** fix the dark curtains: 24.858 → 24.859, while ReSTIR lights the same region 34% brighter from an identical viewpoint. (b) The black angular patches the user reported were **alpha masking, which the engine did not have at all**: Sponza's three `doubleSided` materials are all `alphaMode: MASK` with dark masked texels, so they rasterized as hard-edged dark cards. The cutoff rides the material record's unused word 7; the discard sits after every texture sample because WGSL requires implicit-LOD sampling in uniform control flow. 189,584 pixels change (5% of the frame, max delta 175), on the plants, vines and arcade. (c) **Probes are now placeable by hand**: per-probe offsets stored one texel per probe (a texture, not a buffer — the lookup is a fragment shader and `StorageBuffer` is compute-visible only), read by both the capture, which already pushed `getProbePosition()`, and the lookup, which had to start measuring against the moved position or it compares this frame's distances to somewhere the probe no longer is. `VkmGiSystem::setProbeOffset` is the entry point because moving a probe must also invalidate it — at 0.9 hysteresis the old position's depth would dominate the visibility test for tens of rounds. A `probe_debug` pass draws the grid as octahedrally-shaded impostor spheres (no depth test: a probe buried in a wall is the case the view exists to find) and the gi sample drives an ImGuizmo translate on the selected one. New gate: eight probes rejected on the grid deliver their irradiance once offset to half a unit from the query, sabotage-verified. Metal 254/254. |
