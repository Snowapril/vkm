...
namespace vkm
{
    ...
    /*
    * @brief Owns the engine-wide "bind group 1" per-frame constant set (see
    * common/frame_constants.h).
    * @details One uniform buffer carved into FRAME_COUNT regions plus one bind group per region.
    * Every WebGPU pipeline declares this layout as group 1 alongside the bindless group 0, and
    * every graphics pipeline bind sets it: WebGPU requires every group a pipeline layout declares
    * to be set before a draw, whether or not the shader references it.
    * Unlike group 0's push-constant emulation this uses a static offset per bind group rather than
    * one dynamic-offset group -- the contents change once per frame, not per draw, so there is no
    * offset for a call site to keep in sync.
    */
