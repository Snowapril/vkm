...
namespace vkm
{
    ...
    /*
    * @brief Owns the engine-wide "set 1" per-frame constant set (see common/frame_constants.h).
    * @details One host-visible, persistently mapped uniform buffer carved into FRAME_COUNT regions,
    * plus one descriptor set per region. Every Vulkan pipeline declares this layout as set 1
    * alongside the bindless set 0.
    * Much simpler than VkmBindlessResourceManagerVulkan: the descriptors are written once at
    * initialization and never touched again -- only the buffer bytes change, and only in a region
    * no in-flight frame is reading -- so none of the update-after-bind machinery applies. One
    * static set per frame slot rather than a single UNIFORM_BUFFER_DYNAMIC set keeps every bind
    * site free of dynamic-offset bookkeeping.
    */
